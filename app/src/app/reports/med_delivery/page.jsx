...
export default function MedDeliveryReportPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        setError(null);
        const [historyData, medicineData, stockData] = await Promise.all([
          getDeliveryHistory(),
          getMedicines(),
          getMedicineStock(),
        ]);
        ...
        // Get unique user IDs from doctor_id and delivery_doc_id
        const userIds = new Set();
        historyData.forEach((item) => {
          if (item.doctor_id) userIds.add(item.doctor_id);
          if (item.delivery_doc_id) userIds.add(item.delivery_doc_id);
        });
  ...
  const showMedicineDetails = (medicines, delivery) => {
  ...
  const prepareTableData = () => {
    ...
    const rows = (filteredHistory || []).flatMap(item => {
      return (item.medicine_list || []).map(med => {
        const medDetails = getMedicineDetails(med.med_id, med.med_sid);
        ...
        return [
          ...
          med.quantity || "-",
          medDetails.unit || "-",
          item.status === "pending" ? "รอดำเนินการ" : item.status === "delivering" ? "กำลังจัดส่ง" : "จัดส่งแล้ว",
          doctorName,
          item.note || "-"
        ];
      });
  ...
  const tableToPDF = async () => {
    ...
    try {
      ...
      await exportPDF({
        filename: `delivery_history_report_${new Date().toISOString().split("T")[0]}.pdf`,
        columns,
        rows,
        title: "รายงานประวัติการจัดส่งยา",
  ...
  const tableToCSV = () => {
    ...
    try {
      const { columns, rows } = prepareTableData();
      exportCSV({
        filename: `delivery_history_report_${new Date().toISOString().split("T")[0]}.csv`,
        columns,
        rows,
      });
    } catch (error) {
  ...
  return (
    <div className=" mx-auto font-sarabun">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-4">
        <Header
          header="รายงานประวัติการจัดส่งยา"
          ...
          <div className="bg-white rounded-3xl max-w-3xl w-full max-h-[90vh] overflow-hidden shadow-xl flex flex-col">
            ...
            <div className="px-6 pb-6 overflow-y-auto max-h-[calc(85vh-250px)]">
              <div className="flex items-center gap-4 pb-2 border-b border-gray-200">
                ...
                <p className="text-gray-500">วันที่จัดส่ง {formatDate(selectedDelivery.delivery_date)} เวลา {formattime(selectedDelivery.delivery_date)}</p>
              </div>
              <div className="flex items-center gap-2 relative text-sm py-2">
                <div className="flex items-center justify-start gap-2 pr-2 border-r border-gray-400">
                  ...
                  <span className="font-medium">{`${user[selectedDelivery.doctor_id]?.firstname} ${user[selectedDelivery.doctor_id]?.lastname}` || "-"}</span>
                </div>
              </div>
