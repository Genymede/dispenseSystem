...
export default function DispenseHistoryPage() {
  ...
  const [dispenseDocName, setDispenseDocName] = useState("-");
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        const [historyResult, medicineResult, stockResult, allergyResult] = results;

        if (historyResult.status === "fulfilled") {
          setDispenseHistory(historyResult.value || []);
        } else {
          console.error("Failed to load dispense history:", historyResult.reason);
        }
        ...
        if (allergyResult.status === "fulfilled") {
          setAllergy(allergyResult.value || []);
        } else {
          console.error("Failed to load allergies:", allergyResult.reason);
        }
      } catch (error) {
        console.error("Unexpected error loading data:", error);
        setError("เกิดข้อผิดพลาดขณะโหลดข้อมูล กรุณาลองใหม่");
        Swal.fire({
          icon: "error",
          title: "ข้อผิดพลาด",
          text: "เกิดข้อผิดพลาดขณะโหลดข้อมูล กรุณาลองใหม่",
          confirmButtonText: "ตกลง",
          confirmButtonColor: "#d33",
        });
      } finally {
  ...
  const filteredHistory = useMemo(() => {
    if (!Array.isArray(dispenseHistory)) {
      console.warn("dispenseHistory is not an array:", dispenseHistory);
      return [];
    }
  ...
  const currentItems = Array.isArray(filteredHistory) ? filteredHistory.slice(startIndex, endIndex) : [];
  ...
  const showMedicineDetails = async (medicines, patient) => {
    ...
    try {
      if (patient.doctor_id) {
        const doctorData = await getUserById(patient.doctor_id);
        setDoctorName(`${doctorData.user.firstname || "-"} ${doctorData.user.lastname || "-"}`);
      }
      if (patient.dispense_doc_id) {
        const dispenseDocData = await getUserById(patient.dispense_doc_id);
        setDispenseDocName(`${dispenseDocData.user.firstname || "-"} ${dispenseDocData.user.lastname || "-"}`);
      }
    } catch (error) {
  ...
  return (
    <div className="mx-auto font-sarabun">
      ...
      <div className="mb-4 font-sarabun">
              ...
              <tbody className="divide-y divide-gray-200">
                {currentItems.map((item, index) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <button
                        onClick={() => showMedicineDetails(item.medicines, item)}
                ...
                <div className="pt-2">
                  {[
                    ...
                    <div key={`basic-info-${item.key}`} className="flex items-center justify-start gap-4">
                      ...
                      <span className="font-sm text-gray-900">{item.value}</span>
                    </div>
                  ))}
                </div>
                <div className="pt-2">
                  {[
                    { label: "ผลการวินิจฉัย", value: selectedPatient.description || "-", key: "description" },
                    {
                      ...
                      value:
                        allergy && selectedPatient.patient_id
                          ? allergy
                    ...
                    <div key={`basic-info-${item.key}`} className="flex items-center justify-start gap-4">
                      <span className="text-sm text-gray-600">{item.label}:</span>
                      <span className="font-sm text-red-500">{item.value}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="px-6 pb-6 overflow-y-auto max-h-[calc(85vh-250px)]">
              <div className="flex items-center gap-4 pb-2 border-b border-gray-200">
                ...
                <p className="text-gray-500">
                  วันที่จ่าย <FormatDate dateString={selectedPatient.date} /> เวลา {selectedPatient.time || "-"}
                </p>
              </div>
              <div className="flex items-center gap-2 relative text-sm py-2">
                ...
                <div className="flex items-center justify-start gap-2">
                  ...
                  <span className="font-medium">{dispenseDocName}</span>
                </div>
              </div>
                    ...
                    {selectedMedicines.map((med, index) => {
                      ...
                      return (
                        <tr key={index} className="hover:bg-gray-50 transition-colors">
                          ...
                          <td className="px-4 py-2 text-sm text-gray-900">{med.quantity * medDetails.price}</td>
