...
module.exports = (pool) => {
  ...
  router.get("/problems", async (req, res) => {
    try {
      const result = await pool.query(
        `SELECT * FROM med.med_problem ORDER BY reported_at DESC`
      );
      res.json(result.rows);
    } catch (err) {
      res.status(500).json({ error: err.message });
  ...
  router.get("/problems/:id", async (req, res) => {
    try {
      const { id } = req.params;
      const result = await pool.query(
        `SELECT * FROM med.med_problem WHERE problem_id = $1`,
        [id]
      );
      if (result.rows.length === 0) {
        return res.status(404).json({ error: "ไม่พบปัญหานี้" });
  ...
  router.delete("/problems/:id", async (req, res) => {
    try {
      const { id } = req.params;
      const result = await pool.query(
        `DELETE FROM med.med_problem WHERE problem_id = $1 RETURNING *`,
        [id]
      );
