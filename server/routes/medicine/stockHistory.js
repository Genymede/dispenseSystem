...
module.exports = (pool) => {
  ...
  router.get('/stock_history', async (req, res) => {
    try {
      const result = await pool.query(`SELECT * FROM med.med_stock_history ORDER BY time DESC`);
      res.json(result.rows);
    } catch (err) {
      console.error('🔥 Error loading stock history:', err);
      res.status(500).json({ error: 'Database error' });
  ...
  router.get('/stock_history/:med_id', async (req, res) => {
    ...
    try {
      const result = await pool.query(
        `SELECT * FROM med.med_stock_history WHERE med_id = $1 ORDER BY time DESC`,
        [med_id]
      );
      res.json(result.rows);
    } catch (err) {
  ...
  router.post('/stock_history', async (req, res) => {
    ...
    try {
      const result = await pool.query(
        `
        INSERT INTO med.med_stock_history (med_id, change_type, quantity_change, balance_after, reference_id, time)
        VALUES ($1, $2, $3, $4, $5, NOW())
        RETURNING history_id
        `,
        [med_id, change_type, quantity_change, balance_after, reference_id || null]
