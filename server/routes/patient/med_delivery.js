...
module.exports = (pool) => {
  ...
  router.get("/med_delivery", async (req, res) => {
    try {
      ...
      FROM med.med_delivery d
      LEFT JOIN med.patient p ON d.patient_id = p.patient_id
  ...
  router.delete("/med_delivery/:id", async (req, res) => {
    const { id } = req.params;
    try {
      await pool.query("DELETE FROM med.med_delivery WHERE delivery_id = $1", [id]);
